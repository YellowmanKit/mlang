...
class AddPublish extends View {
  ...
  render(){
    ...
    const school = editMode? this.func.getById.school(this.publish.school, this.store):{};
    const questionnaire = editMode? this.func.getById.questionnaire(this.publish.questionnaire, this.store):{};
    ...
    return(
      <div style={this.viewStyle()}>
        ...
        {this.sep()}
        {this.gap('2%')}
        {questOptions.length > 0 && this.inputs.optionBar('questionnaire', ['50%', this.bs.height * 0.04], questOptions, editMode? questionnaire.title: questOptions[0])}
        ...
        {this.sep()}
        {this.inputs.inputField('schoolCode','text',['Please enter school code','請輸入學校代碼','请输入学校代码'],
        editMode? school.code:'', ()=>{},['25%',''])}
        {this.gap('2%')}

        {this.subTitle(['End date','結束日期','结束日期'])}
        {this.sep()}
        {this.inputs.inputField('endDate','date', ['',''], this.func.dateString(editMode? new Date(this.publish.endDate):defaultDate) ,
        ()=>{ this.setState({modified: true})},['50%',''])}
        {this.gap('2%')}
  ...
  addPublish(){
    ...
    var questionnaire;
    for(var i=0;i<quests.length;i++){ if(quests[i].title === questTitle){ questionnaire = quests[i]; break; } }
